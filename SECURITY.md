...
- Enabling the inactive DeepSeek repair canary intentionally sends bounded text context from controller-granted paths to the customer's selected provider account. The proposal and validation helpers execute only from a trusted-base checkout; the pull-request checkout is treated as data. Observe-only evaluation sends no repository content to a model provider.
