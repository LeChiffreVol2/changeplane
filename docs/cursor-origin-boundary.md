...
This v13 code path is deployed from protected source and exercised against the public canary recorded in `evidence/changeplane-v13-production-release.json`. The exact-head canary published `ChangePlane / guard` from App ID `4334716` / slug `changeplane-guard`; a new commit received a fresh App-owned Check, and a protected test change completed as `action_required` while deterministic CI remained green. That proves the dedicated App/OIDC publication boundary for those exact GitHub revisions and supplies the live policy inputs for Strict Head. It does not complete Tier 2's authenticated `VERIFIED_CURRENT` re-fetch or Queue Certified authority: the available personal-account public repository cannot enable GitHub Merge Queue.
