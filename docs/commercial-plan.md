...
ChangePlane currently distributes a free Apache-2.0 assessment and repository-teamwork runtime for individuals and businesses. The immediate [product and distribution experiment](agentic-product-plan.md) tests reduced PR/CI rescue work with individual maintainers and small teams. Managed behavioral assurance is a future paid-service hypothesis. A Customer Account is the personal account or organization that owns a repository. Larger teams with 20–200 engineers are a later assisted-sales hypothesis, not an account-type eligibility requirement.

For the future hosted service, both account types use the same Verify-first installation, exact-head evidence and protected setup pull request. Enterprise Cloud organizations are included. GitHub Enterprise Server, forks and cross-repository repair remain outside that hosted scope. Private repositories need a GitHub plan that supports the required Rulesets before Strict Head can be claimed; the public read-only assessment does not require Strict Head. Current engineering preparation costs USD 0; the USD 100 monthly ceiling is reserved for scale. See the [account support matrix and phase budget](operating-budget.md).

Verify is the proposed core paid service, subject to the operating gates below. Autonomous Repair is a separately gated expansion and is not required to use the free assessment and handback loop.
