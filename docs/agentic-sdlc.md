...
The dedicated-App-owned `ChangePlane / guard` is ChangePlane's only authoritative guard. The repository job named `ChangePlane guard` is visible operational liveness only and cannot satisfy the guard or Ruleset requirement. Each exact SHA receives one immutable evaluation lease: the owning run and attempt may complete it once, competing runs fail closed, and a rerun after completion invalidates the guard and requires a new commit. GitHub evaluates `merge_group` as a fresh exact revision with a fresh App-owned guard; it inherits no pull-request assurance state.
