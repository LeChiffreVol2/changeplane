...
| Autonomous activation | Implemented and browser-tested: automatic publisher discovery plus exact-SHA, inventory-bound, digest-approved Ruleset plan/apply | Upgrade the production Installer App to Administration write and complete organization-owned Strict Head and Queue Certified activations |
...
| Operations | Implemented and unit-tested: stale Guard reconciliation can only end in `action_required` | Run scheduled synthetic monitoring and reconciliation, publish status, and measure service targets |
