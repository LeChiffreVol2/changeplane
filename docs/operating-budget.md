...
| Supabase Free | Isolated Guard service retained at the approved Free scope; Production connectivity was exercised on synthetic organization publication. Old enrollments are now disabled and occupied authority preserved. See [current observation](current-release.md). Do not change other projects or organization settings. |
| Models, billing and add-ons | No managed model spend, billing service, paid database add-on or new subscription |

The Free project quote is USD 0 per month and the selected Supabase organization remains Free. Both earlier disposable qualification projects have been deleted; [their runtime exercise](../evidence/changeplane-free-runtime-qualification.json) also removed its test credentials and confirmed the existing inventory was unchanged. Following the owner's continuation approval, a new separate Guard service passed [eight runtime checks](../evidence/changeplane-guard-service-qualification.json). Its initial synthetic enrollment remains disabled; a later organization-canary enrollment was activated for qualification and then disabled after the same-SHA defect. The owner subsequently approved retaining the Free service and staging its credential plus explicit CA in Vercel Production. A further [three-check halt drill and staged-secret cleanup](../evidence/changeplane-guard-halt-and-staging.json) completed without deployment; both retained synthetic lanes remain occupied. The separately approved GitHub Free organization and all synthetic repositories were deleted after testing; every enrollment is disabled and four historical authority rows remain retained. Keep synthetic data and test credentials separate from the existing project. At a free quota limit, stop new work; do not upgrade, resume or resize other projects, or remove occupied authority records to continue.
...
The database billing account is selected for evaluation, but an upgrade is deferred until scale and requires a fresh spending decision. Before that change, obtain the authenticated invoice breakdown and review effects on existing projects. The owner created a disposable Free organization; its first canary demonstrated separate live Guard publication and organization Strict Head, but failed same-SHA restart. That repository and its organization App installations have been removed. The v15 hotfix passed same-SHA and manual recovery requalification, and final GitHub cleanup is verified. Automatic ten-minute recovery was not observed; provider authority recovery remains open. See [current evidence](current-release.md). The founder has reported no legal entity; legal documents remain draft. Complete exact-release review, operating drills and customer activation evidence before accepting a payment.
