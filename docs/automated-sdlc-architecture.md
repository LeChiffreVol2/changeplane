...
The next design to validate is a **trusted GitHub Actions publication lane** covering begin, completion and reconciliation. Workflow concurrency alone is insufficient: cancellation must not admit a successor while an earlier external write can still complete. A distributed ref lease without write fencing, another preflight GET, or a process-local mutex on Vercel has the same problem. Immutable per-generation Checks are an alternative only after GitHub's exact required-check selection semantics are proven; do not assume newest-looking metadata is merge authority.

A stricter candidate uses a dedicated writer workflow with an anchored identity and initial run number. Writer N may mutate only after the exact first attempt of N−1 completed successfully under approved default-branch workflow and runtime bytes. A missing, failed, cancelled, rerun or uncertain predecessor stops the lane; it cannot be reset by a timer or by treating missing history as a new beginning. GitHub documents per-workflow run numbers and stable numbers across reruns in its [variables reference](https://docs.github.com/en/actions/reference/workflows-and-actions/variables), and exposes individual attempts through the [workflow-run attempt API](https://docs.github.com/en/rest/actions/workflow-runs#get-a-workflow-run-attempt). These primitives support a candidate protocol, not a GitHub guarantee of serialized Check publication.

The writer's success must acknowledge the actual completed GitHub mutation. A hosted publisher with two accepted copies of one request can otherwise return the first response, admit a newer writer, then apply the delayed second copy. One possible remedy makes Vercel an issuer only and has the isolated trusted writer directly await each bounded Check operation, without retries, redirects or background writes. Duplicate issuer requests then have no Check side effect. This would move repository-scoped Checks-write capability into a trusted runner, require a reviewed exception for a private credential exchange, and require an explicit policy for uncertain writes and fenced recovery. It is not implemented in this candidate. The Guard App key, models and untrusted PR code must remain separate; writer identity must also remain distinct from the source evaluation generation. OIDC's signed job identity can be bound to the actual writer job, but is not a single-use operation claim. See the [OIDC reference](https://docs.github.com/en/actions/reference/security/oidc).
