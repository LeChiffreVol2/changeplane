...
The protected source used for live behavioral qualification is `db4410979ec8c97ec87d92fb7e43f1a43fe56703`, managed v15. The readiness endpoint identifies the current deployed source. The [current release observation](current-release.md) separates completed deployment and initial organization-canary evidence from the same-SHA hotfix that passed live requalification. Neither local tests nor a reviewed source change establish customer readiness.
...
6. **Pilot outcomes can be measured for both GitHub ownership types.** The offline scorecard distinguishes absent evidence from zero incidents, includes pending Guards, rejects duplicate records, and requires a complete 30-day coverage attestation. Schema 2 binds installations/payments to explicit `User` or `Organization` Customer Accounts and reports aggregate cohorts; personal payments cannot satisfy the two-paying-organization target, and incidents from either type count against reliability. Schema 1 organization ledgers retain their existing results. The legacy telemetry projection counts a generation in its earliest retained UTC month; it remains unsuitable for billing.
...
The founder requires **USD 0 now** and reserves **USD 100 per month for scale**. Subsequent continuation approval permitted preparing a separate Free Guard service and an organization canary; paid subscriptions and add-ons remain deferred. The Guard service has passed bounded runtime qualification and a live synthetic enrollment-halt drill. The owner approved keeping it on Free and staging its credential plus explicit CA in Vercel Production; the transfer completed and temporary credential material was removed. That staging snapshot preceded the approved controlled Production rollout and synthetic organization enrollment. The first live run exposed a same-SHA restart defect; its enrollment is now disabled and occupied authority retained. Managed v15 passed live same-SHA, new-head and manual recovery checks. Automatic recovery within ten minutes was not observed. All temporary GitHub resources are deleted and all enrollments disabled; occupied incident authority remains retained. See [current evidence](current-release.md) and the historical [halt and staging evidence](../evidence/changeplane-guard-halt-and-staging.json). The organization and all synthetic repositories were deleted after testing, as the founder expressly required. Verify Lite needs no model key, and billing automation is deferred. Personal and organization GitHub support remain product requirements; GitHub plan eligibility still determines whether a repository can enforce Strict Head. See the [phase budget and hosting eligibility](operating-budget.md).

Qualify runtime authentication, TLS, authority isolation and failure behavior only on the explicitly approved synthetic canary; do not enroll a customer. Delete temporary resources after recording bounded evidence. Free engineering completion does not imply commercial hosting eligibility, durable production authority, organization enforcement, effective legal terms or customer outcomes. Preserve the capability gates until their own evidence exists.
...
The initial disposable Supabase Free project passed [bounded Guard SQL qualification](../evidence/changeplane-supabase-guard-qualification.json). A subsequent disposable project passed [actual runtime login, certificate/hostname validation, pooler isolation and fresh-login credential rotation](../evidence/changeplane-free-runtime-qualification.json) from the operator machine using the existing journal adapter. Both projects were deleted after testing without changing existing projects or the organization plan. The later [isolated Guard service](../evidence/changeplane-guard-service-qualification.json) passed the explicit CA adapter, runtime isolation, concurrent exclusion, finite lane capacity and poison persistence checks with synthetic scope only. Those local observations alone do not establish deployed connectivity or durability. Subsequent [controlled live publication](current-release.md) exercised Vercel connectivity, while provider durability remains unproven. No external customer, payment, effective legal agreement, production failover drill or Origin subscription result has been created by this implementation work.
