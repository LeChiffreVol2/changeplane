...
The managed-v13 candidate operating boundary supports one GitHub.com repository per reviewed setup in `observe`, `verify`, or bounded `autonomous` mode. It is not the currently deployed release and remains subject to the protected canary gates below. A returning owner may repeat that setup for repositories across eligible personal and organization App installations only after release approval. GitHub remains the source of truth, audit surface, and merge authority. ChangePlane has no database, queue, merge service, or proprietary agent runtime to operate.
...
Call Verify Lite active enforcement only when runtime readiness reports `harness.enforcement.active: true`. That read-only state proves at least one independently complete active default-branch Ruleset has no bypass actors, requires strict up-to-date checks and Merge Queue, and binds the dedicated-App `ChangePlane / guard` plus every configured behavioral evidence Check to its expected integration ID. Ruleset ambiguity, bypasses, unsupported targeting, controls split across Rulesets, missing or misbound Checks, and malformed responses fail closed. Classic branch protection is insufficient for this readiness claim. The GitHub-owned `ChangePlane guard` job is operational liveness only, and the installer never mutates either branch-policy surface.
...
5. After deployment, make one read-only Production readiness request. First complete a protected disposable-repository Verify Lite run without BYOK and prove the nine-file manifest, qualifying Ruleset with its dedicated-App guard and publisher-bound evidence Checks, the App begin transition to `in_progress`, same-run/attempt completion, latest workflow-bound exact-head evidence, failed-evidence handback, and fresh-head recheck. Then, as a separate controlled-beta gate, expand that installation to Full through a protected pull request and complete one autonomous run from failed evidence through the same dedicated-App lifecycle on a fresh head. Stop if any payload hash, App-signed grant, ordered marker, clean apply, synchronize event, evidence freshness check, or recheck differs from the verified Preview contract.
...
2. Confirm the GitHub App installation has `administration: read`. Require the selected user to remain a repository admin and require one strict, no-bypass default-branch Ruleset with Merge Queue, the dedicated-App `ChangePlane / guard`, and every configured behavioral evidence Check bound to its expected integration ID. Classic branch protection and the GitHub-owned `ChangePlane guard` liveness job cannot satisfy this gate.
