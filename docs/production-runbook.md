...
| `CHANGEPLANE_DATABASE_URL` | Vercel Production only; separate candidate commercial store | Yes | Commercial PostgreSQL connection with `sslmode=require`; it cannot configure or replace the authority journal. Rotate through the database provider and never log or return it. |
