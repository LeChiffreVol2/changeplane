...
| `CHANGEPLANE_GUARD_REUSE_GITHUB_APP` | Vercel Production migration only | No | Exact `true` keeps a legacy shared Installer/Guard App operational while migration is in progress. Readiness reports `shared_installer_guard` and `commercialReady: false`. Paid rollout requires distinct Guard App variables and a distinct App ID/slug. |
...
| `CHANGEPLANE_COMMERCIAL_STORE_ENABLED` | Vercel Production only | No | Exact `true` enables the reviewed PostgreSQL adapter after schema migration, restore testing, cost alerts, and deletion drill. |
| `CHANGEPLANE_DATABASE_URL` | Vercel Production only | Yes | Managed PostgreSQL connection with `sslmode=require`; rotate through the database provider and never log or return it. |
