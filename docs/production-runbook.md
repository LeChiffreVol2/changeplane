...
## Managed-v15 controlled release boundary

The live behavioral qualification used protected source `db4410979ec8c97ec87d92fb7e43f1a43fe56703`, managed v15. See [the current release evidence and recovery limits](current-release.md); the readiness endpoint identifies the current deployed source. Requested alpha/self-service mode does not grant access. Readiness reports `checks.rolloutAuthorized`, `checks.guardPublicationSerialized`, `checks.commercialRuntimeIntegrated`, `checks.guardJournalConfiguration` and `checks.guardJournalConfigured`; session provides a static `accessBlock` reason and one next action when customer access is paused. Serialization and commercial-runtime capabilities remain false in code, and no environment variable can override them.

The source now wraps begin, completion and reconciliation in a candidate PostgreSQL authority journal with durable exclusive ownership, no expiry takeover, and held/poisoned lanes after uncertain writes. Every `VERCEL=1` Guard write requires it, including owner-canary publication. Missing, disabled, invalid, unavailable or unenrolled journal authority stops the write; there is no unjournaled fallback. Every new canary must bind the dedicated database enrollment and separate Guard App to its exact protected release and epoch.
...
Each installed managed workflow has a separate five-minute reconciliation trigger and manual dispatch inside the same reviewed workflow file. A healthy run reports the number of open exact heads, pending Guard evaluations (including blocked same-head reruns), safe timeout closures, and Guards still inside the window without returning repository names or source. A failed scheduled run is an operational alert: inspect the Actions run and request ID, keep Repair off, and do not weaken the required Guard. Run the manual dispatch during activation and rollback drills. The scheduler is best effort and its observed delay must be included in the monthly pilot report; it is not evidence of a contractual ten-minute SLA.
...
2. Confirm the Installer App installation has `administration: read` for onboarding. Creating an approved Ruleset through ChangePlane additionally requires live `administration: write` on that exact installation; otherwise configure the reviewed policy in GitHub and recheck it. The Guard App must be a distinct principal whose only write authority is Checks. Require the selected user to remain a repository admin. Activate Strict Head from one strict, no-bypass publisher-bound Ruleset; require Queue Certified before Autonomous. Classic branch protection and the GitHub-owned `ChangePlane guard` liveness job cannot satisfy this gate.
