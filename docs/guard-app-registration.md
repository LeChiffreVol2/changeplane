...
The existing Installer App `4334716` was private and could only be installed on its owning personal account. Its registration now permits installation on personal accounts and organizations, following the owner's requested product scope. The approved organization installation `160112795` is restricted to `ChangePlane-Labs/strict-head-canary`; Guard Publisher installation `160110086` is restricted to the same repository. Saved GitHub UI and organization API metadata verified both identities and permission sets. No permission was added to either App registration, no existing personal installation was modified, and no Marketplace listing or customer activation occurred. See [GitHub App visibility](https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/making-a-github-app-public-or-private).

The Installer's OAuth callback is restricted to the exact Production URL `https://changeplane.vercel.app/api/github?action=callback`; wildcard matching is disabled. GitHub had retained wildcard behavior for Apps created before August 3, 2026, while this product supplies one exact callback. The saved setting and a fresh return from GitHub to the exact callback are verified. Current Production redirects to `permissions_required`: its OAuth gate requires Administration write even for Verify, while the actual Installer grants Administration read. The candidate separates Verify onboarding from the optional live Ruleset write grant; this fix is not deployed and an authenticated product session is not yet qualified. See [GitHub callback matching](https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/about-the-user-authorization-callback-url).
