...
**Current status:** The [controlled release observation](current-release.md) supersedes the staging status below. Production uses the separate Guard principal. The original organization canary and its two App installations were removed after a failed same-SHA drill; a replacement using the same separate App passed live v15 same-SHA requalification. All temporary organization repositories and the organization are now deleted; personal installations remain unchanged. Credentials were not replaced or read back.

**Historical staging snapshot:** The separate [ChangePlane Guard Publisher](https://github.com/apps/changeplane-guard-publisher), App ID `4872053`, slug `changeplane-guard-publisher`, is installed on the approved synthetic personal-account canary and the disposable organization canary. [Organization installation evidence](../evidence/changeplane-organization-app-installations.json) records the exact selected repository for both Apps; the organization and repository must be deleted after the live canary. A verified replacement private key is stored as a Production-only Secret in the existing ChangePlane Vercel project; its downloaded file was deleted and the inaccessible first key was revoked after explicit approval. App JWT identity and an exact-repository read token passed live verification; the temporary token was immediately revoked. At that checkpoint the runtime still used the existing Installer/Guard App and reported `shared_installer_guard`. See the final [credential staging evidence](../evidence/changeplane-guard-credential-staging.json). The earlier [registration](../evidence/changeplane-guard-app-registration.json) and [installation](../evidence/changeplane-guard-app-installation.json) records remain immutable snapshots of their respective stages.
...
At the credential-staging checkpoint, the saved secret did not establish runtime activation. No Guard App ID/slug binding, reuse-switch change or redeployment occurred. At that checkpoint, both the protected source and the candidate selected the existing App's complete credential tuple while `CHANGEPLANE_GUARD_REUSE_GITHUB_APP=true`; the staged key is not mixed into that tuple. That checkpoint reported the protected `ddccd7ab7721` controlled canary with shared authority and Repair disabled.

## Organization Installer preparation

The existing Installer App `4334716` was private and could only be installed on its owning personal account. Its registration now permits installation on personal accounts and organizations, following the owner's requested product scope. The initial approved organization installation `160112795` was restricted to `ChangePlane-Labs/strict-head-canary`; Guard Publisher installation `160110086` was restricted to the same repository. Saved GitHub UI and organization API metadata verified both identities and permission sets. No permission was added to either App registration, no existing personal installation was modified, and no Marketplace listing or customer activation occurred. See [GitHub App visibility](https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/making-a-github-app-public-or-private).

The Installer's OAuth callback is restricted to the exact Production URL `https://changeplane.vercel.app/api/github?action=callback`; wildcard matching is disabled. GitHub had retained wildcard behavior for Apps created before August 3, 2026, while this product supplies one exact callback. The saved setting and a fresh return from GitHub to the exact callback are verified. Before PR50, Production redirected to `permissions_required`: its OAuth gate requires Administration write even for Verify, while the actual Installer grants Administration read. PR50 deployed the separation between Verify onboarding and the optional live Ruleset write grant. The original organization canary subsequently qualified OAuth, hosted setup and the manual Ruleset path, as recorded in the current observation. See [GitHub callback matching](https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/about-the-user-authorization-callback-url).
