...
`changeplane_next` (or `team next OWNER/REPO`) reconciles the board and returns only the configured member's pending handoffs. Each handoff names the task, existing workspace, exact head and default-branch revision, findings and one next action. Its digest includes the task generation, trusted policy and current evidence. `changeplane_acknowledge` records receipt with task, workspace ID and handoff ID; it re-reads GitHub before accepting. Receipt never marks a repair successful. Repeated delivery has the same identity; changed code, evidence or default branch creates a new handoff. An unavailable read clears the previous handoff and remains unavailable.
