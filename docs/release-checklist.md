...
## Product 1.0.0 / managed v13 local candidate — not deployed
...
- [ ] Run exact-candidate unit/integration tests, Origin boundary proof, production build, dependency audit, public-data scan, and complete Chromium onboarding and proof journeys on the protected candidate SHA.
...
- [ ] Record GitHub App permission approval, protected source SHA, `CI / verify` run, Preview and Production deployment IDs, readiness release, Verify Lite and Full canary revisions, rollback target, and named launch-window watcher in a new immutable v13 evidence record.

The managed-v13 worktree is not a release, live rollout, broad Rulesets-support claim, active-enforcement claim, or authorization for public Autonomous onboarding. Keep the production repair switch closed until every unchecked v13 activation item has live evidence. The historical v9, v12 freeze, and 2026-07-21 Build Week records below remain evidence only for their exact releases and revisions; their legacy `github-actions` guard results do not prove the v13 dedicated-App lifecycle or Ruleset gate.
