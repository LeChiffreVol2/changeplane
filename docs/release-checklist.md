...
Managed v13 and its dedicated GitHub App publisher are deployed from protected `main`. The live public canary proves exact-head success, publisher identity, stale-head re-evaluation, and protected-evidence refusal. The full high-assurance gate is still inactive because the available personal-account repository cannot enable GitHub Merge Queue; the real Cursor Origin mirror canary is `BLOCKED_NO_SUBSCRIPTION`. See `evidence/changeplane-v13-production-release.json`.
...
- [x] Compute active GitHub readiness only when at least one independently complete active repository or inherited-organization default-branch Ruleset returned by the repository-scoped endpoint has validated parent selector shapes, no bypass actors, strict up-to-date required checks, Merge Queue, the dedicated-App `ChangePlane / guard`, and every behavioral evidence Check bound to its expected integration ID. Fail closed on ambiguous targeting, bypasses, malformed data, controls split across Rulesets, and missing or misbound Checks. Classic branch protection is insufficient; GitHub-owned `ChangePlane guard` is operational liveness only.
...
- [x] Remove App-guard publication authority from repository workflows while retaining the GitHub-owned `ChangePlane guard` job as operational liveness. Require GitHub OIDC for the exact trusted workflow, repository, SHA, event, ref, run ID, and attempt. Before evaluation, create or patch one stable exact-head `ChangePlane / guard` to `in_progress` and supersede older successful App results. At completion, require the same run/attempt lease, re-fetch the managed tree, policy, default branch, same-repository target, and latest eligible exact-head evidence, then mint separate exact-repository read and `checks:write` App credentials. Reject missing begin, stale or older completion, target drift, newer evidence, and malformed ordered markers. Keep readiness and onboarding closed when the dedicated guard or any behavioral evidence publisher cannot be proved in the qualifying Ruleset.
...
- [ ] Exercise Verify Lite end to end without BYOK: review and merge exactly nine files, bind one behavioral Check, publisher, and exact workflow path when the publisher is `github-actions`; observe `ChangePlane guard`; observe the stable App-owned `ChangePlane / guard` transition to `in_progress`; complete it only from the same run/attempt and latest exact-head evidence; return a customer-agent handback on failure; and re-evaluate only after a new exact head is pushed.
...
- [ ] Prove active enforcement against an independently complete strict, no-bypass default-branch Ruleset with Merge Queue, the dedicated-App guard, and every behavioral evidence Check bound to its expected integration ID. Record inactive results for classic-only protection, ambiguous targets, controls split across Rulesets, bypass actors, non-strict policy, missing Merge Queue, missing or misbound App guard, and missing or misbound evidence publishers.
  - Ruleset `21956682` is active, strict, has no bypass actor, and binds `CI / test` to integration `15368` plus `ChangePlane / guard` to integration `4334716`. ChangePlane correctly reports `merge_queue_required`. GitHub rejected the Merge Queue rule with `422` because Merge Queue is unavailable to this personal-account public repository; use an eligible organization-owned public repository for the remaining live gate.
...
- [ ] Approve the dedicated ChangePlane App's required permissions and run a protected disposable-repository v13 canary that records its OIDC claims, GitHub-owned `ChangePlane guard` operational liveness, the App-owned guard's begin `in_progress` state, same-run/attempt completion, stale/older completion rejection, newer-evidence rejection, exact-head proof locator, and `VERIFIED_CURRENT` re-fetch. Confirm one qualifying Ruleset binds the dedicated-App guard and every behavioral evidence Check to the expected integration IDs. Local implementation alone does not complete this release gate.
...
The protected managed-v13 release and dedicated-App publisher are live, but they are not an active high-assurance Merge Queue gate, a real Cursor Origin interoperability result, or authorization for public Autonomous onboarding. Keep the production repair switch closed until every unchecked v13 activation item has live evidence. The historical v9, v12 freeze, and 2026-07-21 Build Week records below remain evidence only for their exact releases and revisions; their legacy `github-actions` guard results do not prove the v13 dedicated-App lifecycle or Ruleset gate.
