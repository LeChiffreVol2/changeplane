...
DeepSeek V4 Flash is the only qualified proposal provider in this release. Its transport is replaceable by reviewed code behind the same patch-only harness; no second provider is supported until it passes the same conformance canary. The durable agentic backbone is the boundary around that worker: a PS256-signed append-only grant primitive intended for the App publisher, campaign-bound deadline, exact controller/base/head and policy binding, protected-path denial, a hard-coded provider endpoint with bounded allowed-path context and no forge write in the proposal job, deterministic clean-job validation, a separate stale-head-checked apply job, and an App-authored push that naturally triggers fresh exact-head checks. The GitHub-hosted proposal runner does not yet enforce a network sandbox, so evidence repair remains inactive. The ledger and verifier are implemented but inactive; live replay resistance is not claimed until the dedicated GitHub App publisher and disposable-repository repair canary pass.

The product preview ends at an observe-only exact-revision receipt and never invokes a model in the browser. The inactive repair path exists only in the controlled lab template after every enforce prerequisite below is satisfied.
