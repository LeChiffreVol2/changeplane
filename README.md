...
Deployed technical baseline: **1.0.0** · managed repository payload: **v14**, under controlled engineering qualification. The [current release observation](docs/current-release.md) records protected deployment, partial organization-canary evidence and a same-SHA hotfix awaiting live requalification. Installed reconciliation, profile-preserving upgrades and the PostgreSQL journal retain their separate operating gates. Customer onboarding remains closed: `guardPublicationSerialized` stays false pending live durability and serialized-canary evidence, followed by exact-release legal approval, separate Guard identity and a Design Partner Alpha repository allowlist. See [automated SDLC architecture and release blockers](docs/automated-sdlc-architecture.md).
